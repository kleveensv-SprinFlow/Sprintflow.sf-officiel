...
COMMENT ON COLUMN public.video_analysis_logs.result_json IS 'JSON object containing the analysis results.';
...
DROP POLICY IF EXISTS "Allow public read access to AI models" ON storage.objects;
DROP POLICY IF EXISTS "Athletes can manage their own videos" ON storage.objects;
DROP POLICY IF EXISTS "Coaches can view their athletes shared videos" ON storage.objects;
