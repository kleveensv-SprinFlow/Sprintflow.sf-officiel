/*
  # Fix Coach Dashboard Analytics Function - Wellness Column Name

  This migration corrects the `get_coach_dashboard_analytics()` function to use the correct
  column name `muscle_fatigue` instead of the incorrect `fatigue_level`.

  ## Changes Made
  - Updated wellness trend calculation to use `muscle_fatigue` column
  - Fixed both the sum calculation and the null-check count
  - Function now correctly calculates average wellness scores for team health metrics

  ## Technical Details
  - The `wellness_log` table has the column `muscle_fatigue` (INT, 1-5 scale)
  - Previous function referenced non-existent `fatigue_level` column
  - This was causing the wellness trend calculation to fail
*/
