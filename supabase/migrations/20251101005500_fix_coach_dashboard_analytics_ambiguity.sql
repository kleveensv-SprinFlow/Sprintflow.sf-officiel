...
BEGIN
  ...
  pending_wellness_data AS (
    ...
    WHERE p.id = ANY(athlete_ids)
      AND NOT EXISTS (
        SELECT 1
        FROM public.wellness_log wl2
        WHERE wl2.user_id = p.id
          AND wl2.date = current_date
      )
  ),
  ...
  SELECT jsonb_build_object(
    'teamHealth', jsonb_build_object(
      'wellnessTrend', COALESCE((SELECT jsonb_agg(wd) FROM wellness_data wd), '[]'::jsonb),
    ...
    'priorityActions', jsonb_build_object(
      'pendingWellness', COALESCE((SELECT jsonb_agg(pwd) FROM pending_wellness_data pwd), '[]'::jsonb),
      'pendingValidation', COALESCE((SELECT jsonb_agg(pvd) FROM pending_validation_data pvd), '[]'::jsonb)
    )
  )
  INTO result;
