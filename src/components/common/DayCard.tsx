...
interface DayCardProps {
  date: Date;
  workouts: Workout[]; // Modifié pour accepter plusieurs entraînements
...
const WorkoutSummary: React.FC<{ workout: Workout }> = ({ workout }) => {
  // ... (Le contenu de ce composant ne change pas)
...
export const DayCard: React.FC<DayCardProps> = ({ date, workouts, onPlanClick, onEditClick, isReadOnly = false }) => {
  const getDayLabel = () => {
  ...
  return (
    <div className="w-full h-full rounded-2xl p-4 flex flex-col justify-between bg-light-card dark:bg-dark-card shadow-card-light dark:shadow-card-dark transition-all duration-300 group">
      <header className="flex justify-between items-start">
        ...
        {!isReadOnly && onEditClick && workouts.length > 0 && (
          <button onClick={() => onEditClick(workouts[0].id)} className="p-2 rounded-full transition-all bg-black/5 dark:bg-white/10 opacity-0 group-hover:opacity-100 hover:bg-black/10 dark:hover:bg-white/20">
      ...
      <div className="flex-grow flex items-center justify-center">
        {workouts.length > 0 ? (
          <div className="space-y-3 text-light-text dark:text-dark-text">
