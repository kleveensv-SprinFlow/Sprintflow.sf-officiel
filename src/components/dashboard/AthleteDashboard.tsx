...
const AthleteDashboard: React.FC = () => {
  ...
  const [planningView, setPlanningView] = useState<{isOpen: boolean, mode: 'planned' | 'completed'}>({isOpen: false, mode: 'planned'});
    ...
    <div className="space-y-6 p-4">
      ...
      {groups.length === 0 && (
        <div 
          ...
          className="p-5 bg-gradient-to-br from-indigo-600 to-indigo-700 rounded-2xl shadow-lg shadow-indigo-500/20 flex items-center justify-between cursor-pointer active:scale-95 transition-all group"
        >
      ...
      <div onClick={() => setPlanningView({isOpen: true, mode: 'planned'})}>
        <TodaysWorkoutCard />
      </div>
      <div onClick={() => setPlanningView({isOpen: true, mode: 'completed'})}>
        <LastWorkoutCard />
      </div>
      ...
      <WeeklyPlanningView
        isOpen={planningView.isOpen}
        onClose={() => setPlanningView({isOpen: false, mode: 'planned'})}
        viewMode={planningView.mode}
      />
    </div>
  );
};
