...
interface IndicesPanelProps {
  ...
  loading?: boolean;
  onFormCheckinClick?: () => void; // Handler specifically for the Form card
}
...
const GlassCard = ({ children, className = '', onClick }: { children: React.ReactNode; className?: string; onClick?: () => void }) => (
  <div 
    onClick={onClick}
    className={`relative overflow-hidden rounded-3xl bg-white/10 backdrop-blur-md border border-white/20 shadow-xl ${onClick ? 'cursor-pointer active:scale-95 transition-transform' : ''} ${className}`}
  >
...
const IndicesPanel: React.FC<IndicesPanelProps> = ({
  ...
  return (
    <div className="grid grid-cols-2 gap-4 w-full">
      {/* Left Card: Form Index (Clickable if checkin needed) */}
      <GlassCard 
        onClick={!isCheckinDone ? onFormCheckinClick : undefined}
        className={`h-40 flex flex-col justify-between p-5 transition-all duration-300 ${!isCheckinDone ? 'brightness-75 hover:brightness-90' : ''}`}
      >
        {!isCheckinDone ? (
      ...
      {/* Right Card: Performance Index (Independent) */}
      {/* TODO: Add onClick for Performance details if needed later */}
