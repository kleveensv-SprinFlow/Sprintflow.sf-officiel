...
export function NewWorkoutForm({ userRole, onSave, onCancel, initialData }: NewWorkoutFormProps) {
  ...
  return (
    <>
      ...
      />
      <motion.div
        className="fixed bottom-0 left-0 right-0 z-50 h-[95vh] max-h-[1000px] bg-gray-800/60 dark:bg-black/50 backdrop-blur-xl rounded-t-3xl shadow-2xl border-t border-white/10 flex flex-col"
      ...
      >
        <div className="flex-shrink-0 flex items-center justify-between p-4 border-b border-white/10">
          ...
          <h2 className="text-xl font-bold text-white text-center">{modalTitle}</h2>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-6 no-scrollbar">
          ...
          {workoutType !== 'manuscrit' && (
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">Contenu de la séance *</label>
              {!isCompletingWorkout && (
          ...
          {workoutType === 'manuscrit' && (
            <textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={10} className="w-full p-3 rounded-lg bg-white/5 border border-white/10 placeholder-gray-400 text-white" placeholder="Décrivez la séance..."/>
          )}
          
          <div className="pt-4 space-y-4">
            {!showNotes && <button type="button" onClick={() => setShowNotes(true)} className="w-full flex items-center justify-center gap-2 text-sm text-blue-400 font-semibold"><MessageSquarePlus size={16}/>Ajouter une note</button>}
            <AnimatePresence>
              {showNotes && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}>
                  <textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={4} className="w-full p-3 rounded-lg bg-white/5 border border-white/10 placeholder-gray-400 text-white" placeholder="Note globale..."/>
                </motion.div>
              )}
            </AnimatePresence>
            <div className="flex flex-wrap gap-3 justify-end pt-4 border-t border-white/10">
              ...
              <div className="flex-grow flex gap-3">
                  <button type="button" onClick={onCancel} className="w-full px-6 py-3 bg-gray-600/50 hover:bg-gray-600 text-white rounded-xl transition-colors">Annuler</button>
