...
import { ArrowRight, Users, User, Check, Loader2, X } from 'lucide-react';
...
export const JoinGroupModal: React.FC<JoinGroupModalProps> = ({ onClose, onSuccess }) => {
  ...
  const handleJoin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(error.message || 'Code invalide ou erreur lors de l\'adhésion.');
    } finally {
      setLoading(false);
    }
  };
        ...
        <form onSubmit={handleJoin} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-3 bg-gray-50 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none transition-all font-mono text-center uppercase tracking-widest text-lg"
              placeholder="A1B2C3"
              maxLength={6}
              required
            />
            <p className="text-xs text-gray-500 mt-2 text-center">
              Demandez ce code à votre coach.
            </p>
          </div>
