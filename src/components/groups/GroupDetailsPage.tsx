...
export const GroupDetailsPage: React.FC<GroupDetailsPageProps> = ({ group, onBack, onViewAthlete }) => {
  ...
  // On récupère leaveGroup en plus de fetchJoinRequests et respondToRequest
  const { fetchJoinRequests, respondToRequest, leaveGroup } = useGroups();
  const isAthlete = profile?.role === 'athlete';

  // Charge les infos du coach
  useEffect(() => {
  ...
  // Quitte le groupe (pour les athlètes)
  ...
  return (
    <div className="p-4 space-y-4 animate-in fade-in-0">
      {/* En-tête : nom du groupe, code et bouton de retour/quitter */}
      <div className="flex items-center space-x-2">
        ...
        {isAthlete && (
          <button
            ...
            className="ml-auto px-3 py-1 rounded-md bg-red-500 text-white hover:bg-red-600 transition"
          >
            Quitter le groupe
          </button>
        )}
      ...
      {/* Tabs pour le coach : Membres / Demandes */}
      {!isAthlete && (
        <div className="border-b border-gray-200 dark:border-gray-700">
          <nav className="flex space-x-4">
            <button
              onClick={() => setActiveTab('members')}
              className={`flex items-center space-x-2 py-3 px-1 border-b-2 font-medium ${
                activeTab === 'members'
      ...
      {/* Contenu : liste des membres ou des demandes */}
      <div>
        {(activeTab === 'members' || isAthlete) && (
          <div className="space-y-3">
                ...
                <div className="relative">
                  <img
                    src={
                      ...
                      `https://ui-avatars.com/api/?name=${coachProfile.first_name}+${coachProfile.last_name}&background=f97316`
                    }
                    alt="avatar"
            ...
            {group.group_members.length > 0 ? (
                ...
                >
                  <img
                    src={
                      ...
                      `https://ui-avatars.com/api/?name=${member.profiles?.first_name}+${member.profiles?.last_name}&background=3b82f6`
                    }
                    alt="avatar"
                    className="w-12 h-12 rounded-full object-cover"
                  />
              ...
              <p className="text-center py-8 text-gray-500">Aucun membre dans ce groupe.</p>
            ) : null}
          </div>
        )}
        ...
        {!isAthlete && activeTab === 'requests' && (
          <div className="space-y-3">
            {loadingRequests ? (
              ...
              <p className="text-center py-8 text-gray-500">Aucune demande en attente.</p>
            ) : (
              requests.map(req => (
                <div
                  key={req.id}
                  ...
                  <div className="flex items-center space-x-3">
                    <img
                      src={
                        ...
                        `https://ui-avatars.com/api/?name=${req.profiles?.first_name}+${req.profiles?.last_name}&background=random`
                      }
                      alt="avatar"
                    ...
                    {processingId === req.id ? (
                      ...
                      <>
                        <button
                          ...
                          className="p-2 bg-red-100 dark:bg-red-900/50 text-red-600 dark:text-red-400 rounded-full hover:bg-red-200"
                        >
                          <X size={18} />
                        </button>
                        <button
                          onClick={() => handleResponse(req.id, 'accepted')}
                          className="p-2 bg-green-100 dark:bg-green-900/50 text-green-600 dark:text-green-400 rounded-full hover:bg-green-200"
                        >
                          <Check size={18} />
                        </button>
                      </>
